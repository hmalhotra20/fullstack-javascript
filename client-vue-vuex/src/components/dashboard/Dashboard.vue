      ...
      <div class="w3-row w3-padding-32" v-for="data in realtimeDataForSymbols"
        ...
        <div class="w3-third w3-small w3-border w3-padding" style>
          ...
          <div class="w3-container w3-row"
            ...
            <div class="w3-text-green w3-half">:
              {{ data[key] }}
            </div>
          </div>
        </div>
        <div class="w3-half w3-small w3-center" v-if="historicalData && data.symbol">
          <chart v-bind:historyForSymbol="historicalData[data.symbol]"></chart>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  created() {
    this.registeredSymbols = this.$store.state.tickers;
  },
  data() {
    return {
      registeredSymbols: []
